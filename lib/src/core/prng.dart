...
class _PRNGImpl extends PRNG {
  ...
  int _integer(int min, int max) {
    final nextNb = next();
    // Convert the 32-bit signed integer to a value between [0, 1)
    final double normalized = (nextNb - PRNG.min) / (PRNG.max - PRNG.min + 1.0);
    // Scale to the desired range [min, max)
    return min + (normalized * (max - min)).floor();
  }
